...
def search_komputronik(product):
    with sync_playwright() as p:
        ...
        url = "https://www.komputronik.pl/search/category/?query=" + search
        page.goto(url)
        ...
        for locator in list_[1:]:
            data = locator.locator('[class="product-image"]').inner_html()
            ...
            if product.lower() in name.lower():
                ...
                prize = locator.locator('[class="text-3xl font-bold leading-8"]').first.text_content()
                price_str = prize.replace('\xa0', '')
                price_str = price_str.replace(',', '.')
                price_str = price_str.replace('zł', '').strip()
                price_float = float(price_str)
                items.append({"name": name, "link": href_value, "price": price_float, "src": src})
...
def search_euro_agd(product):
    with sync_playwright() as p:
        ...
        offers_list_div = page.locator(
            'body > ems-root > eui-root > eui-dropdown-host > div.content > ems-euro-mobile > ems-euro-mobile-product-listings > div > ems-euro-mobile-product-list-wrapper > ems-euro-mobile-product-list > div > div > section > ems-product-list-results > div > ems-euro-mobile-product-medium-box').all()
        page.wait_for_timeout(3000)
        items = []
        refs = []
        ...
        for i in offers_list_div:
            ...
            src_tag = soup.find('img', class_='product-medium-box-content__photo')
            src = src_tag['src']

            a_tag = soup.find('a')
            href_value = a_tag['href']
            price = i.locator('[class="parted-price"]').text_content()
            price_str = price.replace("zł", "").replace("\u0142", "").replace(" ", "")
            ...
            items.append({"name": a_tag.text, "link": base_url + href_value, "price": price_float, "src": src})
            page.wait_for_timeout(3000)
...
def search_morele_net(product_):
    ...
    if response.status_code == 200:
        ...
        for product in products:
            ...
            price_text = price_text.replace('zł', '').replace('\xa0', '')
            ...
            items.append({"name": product_name, "link": full_link, "price": price_value, "src": src})
